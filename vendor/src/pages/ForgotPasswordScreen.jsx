      ...
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl border border-gray-100 p-8">
        ...
        {message ? (
          <div className="text-center">
            ...
            <Link to="/login" className="inline-block mt-4 text-primary hover:text-pink-600 font-bold text-sm transition-colors">
              Return to Login
            </Link>
          </div>
        ) : (
          <form onSubmit={submitHandler} className="space-y-5">
            ...
            <button 
              type="submit" 
              disabled={loading}
              className="w-full bg-primary text-white font-bold py-3 rounded-xl hover:bg-secondary transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed flex justify-center items-center h-12"
            >
              {loading ? (
